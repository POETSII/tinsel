#ifndef host_logging_hpp
#define host_logging_hpp
...
private:
    ...
    void do_exit_region(const char *name)
    {
        double t=now();
        m_tLastProgress=t;
        ...
        if(name!=nullptr){
            if(m_regions.back().local_name!=name){
                fatal_error("Attempt to exit region called '%g', but current region is '%g'", name, m_regions.back().local_name.c_str());
            }
        }
...
public:
    ...
    HostLogger(std::string log_file_name="")
        : m_dst(0)
    {
    ...
    void hook_graph(PGraph<DeviceType,S,E,M>  &graph)
    {
        graph.on_phase_hook=[&](int direction, const char *name) {
            if(direction==+1){
                enter_region(name);
            }else if(direction==-1){
                exit_region(name);
        ...
        graph.on_fatal_error=[&](const char *msg) { fatal_error(msg); };
        graph.on_export_value=[&](const char *name, double value) { export_value(name, value, 2); };
        graph.on_export_string=[&](const char *name, const char * value) { export_value(name, value, 2); };
    }
    ...
    void exit_region(const char *name)
    {
        std::unique_lock<std::recursive_mutex> lk(m_mutex);
    ...
    HostLogContext with_region(const char *name);
};

struct HostLogContext
{
private:
    std::string m_name;
    HostLogger *m_logger=0;
public:
    HostLogContext()
    {}

    HostLogContext(const HostLogContext &o) = delete;

    HostLogContext(HostLogContext &&o)
    ...
    HostLogContext &operator=(const HostLogContext &) = delete;

    HostLogContext &operator=(HostLogContext &&o)
    {
        assert(!m_logger);
    ...
    HostLogContext(HostLogger &logger, const char *name)
    {
        enter(logger, name);
    }

    void enter(HostLogger &logger, const char *name)
    {
        assert(!m_logger);
        logger.enter_region(name);
        m_name=name;
        m_logger=&logger;
    }
    ...
    {
        assert(m_logger);
        m_logger->exit_region(m_name.c_str());
        m_logger=0;
    }
...
HostLogContext HostLogger::with_region(const char *name)
{
    return HostLogContext(*this, name);
}
