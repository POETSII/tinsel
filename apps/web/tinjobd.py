...
while 1:
  ...
  if len(queue) > 0:
    jobId = queue[0]
    if state == idleState:
      resetProc = subprocess.Popen("make -C ../../de5 download-sof",
                    shell=True)
      state = resetState
    elif state == resetState and resetProc.poll() is not None:
      if resetProc.returncode != 0:
        jobResult[jobId] = "E000022FPGA Programming Error"
        ...
        f.write(job[jobId])
        f.close()
        runProc = subprocess.Popen("make " +
                    str(jobId) + "-fpga.png.64",
                    shell=True)
        state = runState
    elif state == runState and runProc.poll() is not None:
      if not os.path.isfile(str(jobId) + "-fpga.png.64"):
        ...
        imgFile = open(str(jobId) + "-fpga.png.64", "r")
