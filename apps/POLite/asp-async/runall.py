...
for ns in NUM_SOURCES:
    for x_len in X_LEN:
        for y_len in Y_LEN:
            run_command("NUM_SOURCES={} X_LEN={} Y_LEN={} make all".format(ns, x_len, y_len))
            run_command("scp -r build byron:asp")

            for x,y in cube_sizes:
                for i in range(2):
                    run_command("""ssh -t byron "cd asp; source /local/ecad/setup-quartus17v0.bash; ./build/run edges_{x}_{y}.txt >> results{i}.json" """.format(x=x,y=y,i=RANDOM_ID))
