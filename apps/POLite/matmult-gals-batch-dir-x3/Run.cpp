...
int main() {
    ...
    if (!mult_possible) {
        ...
        for (uint32_t x = 0; x < MESHLEN; x++) {
            for (uint32_t y = 0; y < MESHWID; y++) {
                for (uint32_t z = 0; z < MESHHEI; z++) {
                    ...
                    graph->devices[mesh[x][y][z]]->state.aggregate[0][2] = 0;
                    graph->devices[mesh[x][y][z]]->state.aggregate[1][0] = 0;
                    graph->devices[mesh[x][y][z]]->state.aggregate[1][1] = 0;
                    graph->devices[mesh[x][y][z]]->state.aggregate[1][2] = 0;
                    graph->devices[mesh[x][y][z]]->state.aggregate[2][0] = 0;
                    graph->devices[mesh[x][y][z]]->state.aggregate[2][1] = 0;
                    graph->devices[mesh[x][y][z]]->state.aggregate[2][2] = 0;
                    
                    if (x == 0) {
                        graph->devices[mesh[x][y][z]]->state.elementX[0][0] = matrixA[(y*3)][(z*3)];
                        graph->devices[mesh[x][y][z]]->state.elementX[0][1] = matrixA[(y*3)+1][(z*3)];
                        graph->devices[mesh[x][y][z]]->state.elementX[0][2] = matrixA[(y*3)+2][(z*3)];
                        graph->devices[mesh[x][y][z]]->state.elementX[1][0] = matrixA[(y*3)][(z*3)+1];
                        graph->devices[mesh[x][y][z]]->state.elementX[1][1] = matrixA[(y*3)+1][(z*3)+1];
                        graph->devices[mesh[x][y][z]]->state.elementX[1][2] = matrixA[(y*3)+2][(z*3)+1];
                        graph->devices[mesh[x][y][z]]->state.elementX[2][0] = matrixA[(y*3)][(z*3)+2];
                        graph->devices[mesh[x][y][z]]->state.elementX[2][1] = matrixA[(y*3)+1][(z*3)+2];
                        graph->devices[mesh[x][y][z]]->state.elementX[2][2] = matrixA[(y*3)+2][(z*3)+2];
                    }
                    if (y == 0) {
                        graph->devices[mesh[x][y][z]]->state.elementY[0][0] = matrixB[(z*3)][(x*3)];
                        graph->devices[mesh[x][y][z]]->state.elementY[0][1] = matrixB[(z*3)][(x*3)+1];
                        graph->devices[mesh[x][y][z]]->state.elementY[0][2] = matrixB[(z*3)][(x*3)+2];
                        graph->devices[mesh[x][y][z]]->state.elementY[1][0] = matrixB[(z*3)+1][(x*3)];
                        graph->devices[mesh[x][y][z]]->state.elementY[1][1] = matrixB[(z*3)+1][(x*3)+1];
                        graph->devices[mesh[x][y][z]]->state.elementY[1][2] = matrixB[(z*3)+1][(x*3)+2];
                        graph->devices[mesh[x][y][z]]->state.elementY[2][0] = matrixB[(z*3)+2][(x*3)];
                        graph->devices[mesh[x][y][z]]->state.elementY[2][1] = matrixB[(z*3)+2][(x*3)+1];
                        graph->devices[mesh[x][y][z]]->state.elementY[2][2] = matrixB[(z*3)+2][(x*3)+2];
                    }
                }
            }
        }
        ...
        uint32_t result[(MESHLEN*3)][(MESHWID*3)] {};
        ...
        for (uint32_t i = 0; i < (RETMATSIZE + 1); i++) {
            ...
            if (msg.payload.dir != CCNTDIR) {
                result[(graph->devices[msg.payload.dir]->state.x)*3][(graph->devices[msg.payload.dir]->state.y)*3] = msg.payload.val[0];
                result[((graph->devices[msg.payload.dir]->state.x)*3)+1][(graph->devices[msg.payload.dir]->state.y)*3] = msg.payload.val[3];
                result[((graph->devices[msg.payload.dir]->state.x)*3)+2][(graph->devices[msg.payload.dir]->state.y)*3] = msg.payload.val[6];
                result[(graph->devices[msg.payload.dir]->state.x)*3][((graph->devices[msg.payload.dir]->state.y)*3)+1] = msg.payload.val[1];
                result[((graph->devices[msg.payload.dir]->state.x)*3)+1][((graph->devices[msg.payload.dir]->state.y)*3)+1] = msg.payload.val[4];
                result[((graph->devices[msg.payload.dir]->state.x)*3)+2][((graph->devices[msg.payload.dir]->state.y)*3)+1] = msg.payload.val[7];
                result[(graph->devices[msg.payload.dir]->state.x)*3][((graph->devices[msg.payload.dir]->state.y)*3)+2] = msg.payload.val[2];
                result[((graph->devices[msg.payload.dir]->state.x)*3)+1][((graph->devices[msg.payload.dir]->state.y)*3)+2] = msg.payload.val[5];
                result[((graph->devices[msg.payload.dir]->state.x)*3)+2][((graph->devices[msg.payload.dir]->state.y)*3)+2] = msg.payload.val[8];
        ...
        for (uint32_t y = 0; y < (MESHWID*3); y++) {
            for (uint32_t x = 0; x < (MESHLEN*3); x++) {
