...
int main() {
    ...
    if (!mult_possible) {
        ...
        for (uint32_t w = 0; w < MESHWID; w++) {
            for (uint32_t l = 0; l < MESHLEN; l++) {
                ...
                // From maxtrix A
                deviceAddr = graph.toDeviceAddr[mesh[0][l][w]];
                //printf("deviceAddr = %d\n", deviceAddr);
                sendMsg.devId = getLocalDeviceId(deviceAddr);
                sendMsg.payload.from = EXTERNALX;
                sendMsg.payload.element1 = matrixA[(MESHLEN-1)-l][w];
                //printf("%d \n ", matrixA[l][w]);
    ...
    /*
        ...
        uint32_t aggregate[MESHWID][MESHLEN][MESHHEI] {};
        uint32_t source_x[MESHWID][MESHLEN][MESHHEI] {};
        uint32_t source_y[MESHWID][MESHLEN][MESHHEI] {};
        uint32_t source_z[MESHWID][MESHLEN][MESHHEI] {};
        uint32_t element1[MESHWID][MESHLEN][MESHHEI] {};
        uint32_t element2[MESHWID][MESHLEN][MESHHEI] {};
        //uint32_t id[MESHWID][MESHLEN][MESHHEI] {};
