...
inline bool get_option_bool(const char *name, bool def) 
{
    auto *p=getenv(name);
    if(p){
        std::string s(p);
...
inline unsigned get_option_unsigned(const char *name, unsigned def)
{
    auto *p=getenv(name);
    if(p){
        try{
...
private:
    ...
    void worker_proc()
    {
        ...
        std::function<void (void *,size_t)> send_cb=[&](void *p, size_t n)
        {
            ...
            std::vector<uint8_t> tmp(1<<TinselLogBytesPerMsg, 0);
            memcpy(&tmp[0], p, n);
...
public:
    ...
    {
        if(numBytes > (1<<TinselLogBytesPerMsg)){
        ...
        const int MaxMessageSize=1<<TinselLogBytesPerMsg;
    ...
    void recv(void* msg)
    {
        recvMsg(msg, 1<<TinselLogBytesPerMsg);
    }
};
